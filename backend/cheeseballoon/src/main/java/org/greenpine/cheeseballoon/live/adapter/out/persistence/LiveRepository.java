...
//    @Query("SELECT LiveEntity FROM LiveEntity le where le.streamer.streamerId = :streamerId order by le.liveId ")
    ...
    @Query(value = "SELECT ROW_NUMBER() OVER(ORDER BY averageViewer DESC) rank, ranksql.streamer_id AS streamerId, averageViewer, profileUrl, platform, ranksql.name, case when bookmark_id IS NULL then 'false' ELSE 'true' END AS bookmark FROM \n" +
            "(SELECT live.streamer_id, round(avg(live_log.viewer_cnt),0) AS averageViewer, s.name, s.platform, s.profile_url AS profileUrl\n" +
            "FROM lives AS live\n" +
            ...
            "(SELECT * FROM bookmarks WHERE member_id = :memberId) AS b ON ranksql.streamer_id = b.streamer_id", nativeQuery = true)
    List<FindAvgViewerRankResDtoInterface> findAllAvgViewerRanking(LocalDateTime beforeDay, LocalDateTime today, Long memberId);

    // 특정 플랫폼에서 평균 시청자 수 기준으로 가져오는 sql
    @Query(value = "SELECT ROW_NUMBER() OVER(ORDER BY averageViewer DESC) rank, ranksql.streamer_id AS streamerId, averageViewer, profileUrl, platform, ranksql.name, case when bookmark_id IS NULL then 'false' ELSE 'true' END AS bookmark FROM \n" +
            "(SELECT live.streamer_id, round(avg(live_log.viewer_cnt),0) AS averageViewer, s.name, s.platform, s.profile_url AS profileUrl\n" +
            "FROM lives AS live\n" +
            "inner join (SELECT * FROM live_logs WHERE cycle_log_id in (SELECT cycle_log_id FROM cycle_logs WHERE cycle_dt BETWEEN :beforeDay AND :today))\n" +
    ...
    @Query(value = "SELECT ROW_NUMBER() OVER(ORDER BY topViewer DESC) rank, ranksql.streamer_id AS streamerId, topViewer, profileUrl, platform, ranksql.name, case when bookmark_id IS NULL then 'false' ELSE 'true' END AS bookmark FROM \n" +
            "(SELECT live.streamer_id, MAX(live_log.viewer_cnt) AS topViewer, s.name, s.platform, s.profile_url AS profileUrl\n" +
            "FROM lives AS live\n" +
            "inner join (SELECT * FROM live_logs WHERE cycle_log_id in (SELECT cycle_log_id FROM cycle_logs WHERE cycle_dt BETWEEN :beforeDay AND :today))\n" +
            "live_log ON live.live_id = live_log.live_id, streamers s\n" +
            "WHERE s.streamer_id = live.streamer_id\n" +
